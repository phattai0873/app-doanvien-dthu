...
import { Home, Briefcase, Bell, User, Calendar, FileText, ChevronRight, Menu, Database, Users, Wallet, Award, BookOpen, Clock, Filter, CheckCircle, AlertCircle, Library, GraduationCap } from 'lucide-react';
  ...
  notifications: [
    {
      ...
      content: "Kính mời các đồng chí tham dự cuộc họp định kỳ tháng 6...",
    ...
    },
    {
      ...
      content: "Hệ thống ghi nhận đồng chí chưa hoàn thành nghĩa vụ...",
...
const App = () => {
  const [activeTab, setActiveTab] = useState('work'); // Mặc định ở trang Công tác để kiểm tra
  ...
  const renderContent = () => {
    switch (activeTab) {
      case 'news':
        return <NewsFeed user={DB.user} news={DB.featured_news} notifs={DB.notifications} />;
      case 'work':
    ...
    <div className="p-5 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <WorkCard 
          ...
          desc="Các cuộc thi định kỳ" 
          table="quiz_exams" 
        />

        {/* 4. Tài liệu học tập (NEW SPLIT: DOCUMENTS) */}
        <WorkCard 
          icon={<Library size={32} className="text-teal-600" />} 
          bgIcon="bg-teal-100" 
          title="Kho Tài liệu" 
          desc="Văn kiện, Nghị quyết (File)" 
          table="documents" 
        />

        {/* 5. Học tập lý luận (NEW SPLIT: POLITICAL STUDIES) */}
        <WorkCard 
          icon={<GraduationCap size={32} className="text-purple-600" />} 
          bgIcon="bg-purple-100" 
          title="Học tập Lý luận" 
          desc="Lớp bồi dưỡng, Quán triệt" 
          table="political_studies" 
        />
      </div>
    </div>
  );
...
// ... (Giữ nguyên các component con khác: NewsFeed, NotificationScreen, FilterTab, WorkCard, NavItem)
        ...
        {filteredList.length > 0 ? (
          filteredList.map(item => (
            <div key={item.id} className={`bg-white p-4 rounded-xl border border-gray-100 shadow-sm relative overflow-hidden transition-all active:scale-[0.98] ${!item.is_read ? 'bg-red-50/30' : ''}`}>
              {!item.is_read && <div className="absolute top-4 right-4 w-2 h-2 bg-[#EE0033] rounded-full shadow-sm animate-pulse"></div>}
              <div className="flex gap-4">
                <div className="flex flex-col items-center gap-2">
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${item.type === 'meeting' ? 'bg-red-100 text-[#EE0033]' : item.type === 'fee' ? 'bg-orange-100 text-orange-600' : 'bg-blue-100 text-blue-600'}`}>
                    {item.type === 'meeting' && <Users size={20} />}
                    {item.type === 'fee' && <Wallet size={20} />}
                    {item.type === 'quiz' && <Award size={20} />}
                    {item.type === 'doc' && <FileText size={20} />}
                  </div>
                  {item.is_urgent && <span className="text-[9px] font-bold text-white bg-[#EE0033] px-1.5 py-0.5 rounded shadow-sm">GẤP</span>}
                </div>
                <div className="flex-1">
                  <h3 className={`text-sm font-bold leading-snug mb-1 ${!item.is_read ? 'text-gray-900' : 'text-gray-700'}`}>{item.title}</h3>
                  <p className="text-xs text-gray-500 line-clamp-2 mb-2">{item.content}</p>
                  <div className="flex items-center justify-between text-[10px] text-gray-400">
                    <div className="flex items-center gap-1"><User size={10} /><span className="font-medium text-gray-500">{item.sender}</span></div>
                    <div className="flex items-center gap-1"><Clock size={10} /><span>{item.time}</span></div>
                  </div>
                </div>
              </div>
              <div className="absolute bottom-1 right-2 opacity-20 text-[8px] font-mono">{item.source_note}</div>
            </div>
          ))
        ) : (
          <div className="flex flex-col items-center justify-center py-10 text-gray-400"><CheckCircle size={40} className="mb-2 opacity-50"/><p className="text-sm">Không có thông báo nào</p></div>
        )}
      </div>
    </div>
  );
...
const NewsFeed = ({ news, notifs }) => (
  <div className="pt-6 px-4 space-y-6 animate-in fade-in duration-500">
    ...
    <section>
      <div className="flex justify-between items-center mb-3 pl-1 border-l-4 border-[#EE0033]">
        <h2 className="text-gray-800 font-bold text-lg uppercase leading-none">Thông báo mới</h2>
        <span className="text-[10px] font-mono text-gray-400 bg-gray-100 px-1 rounded">Table: notifications</span>
      </div>
        ...
        {notifs.map(item => (
          <div key={item.id} className="bg-white p-3 rounded-xl border border-gray-100 flex gap-3 items-center shadow-sm relative overflow-hidden group">
            ...
            <div className="flex-1 min-w-0 pr-4">
              <h4 className="text-xs font-mono font-bold text-blue-600 bg-blue-50 p-1 rounded border border-blue-100 border-dashed inline-block mb-1">{item.title}</h4>
              <div className="flex items-center gap-2"><p className="text-[10px] text-gray-500 font-mono bg-gray-100 px-1 rounded">{item.time}</p></div>
            </div>
          </div>
        ))}
      </div>
