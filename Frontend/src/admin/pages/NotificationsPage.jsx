...
const PRIORITY_CONFIG = {
    'Thấp': { cls: 'bg-gray-100 text-gray-600' },
    'Trung bình': { cls: 'bg-blue-50 text-blue-700' },
    'Cao': { cls: 'bg-orange-50 text-orange-700' },
};
const STATUS_CONFIG = {
    'Mới': { cls: 'bg-yellow-50 text-yellow-700' },
    'Đã gửi': { cls: 'bg-green-50 text-green-700' },
    'Đã hủy': { cls: 'bg-red-50 text-red-700' },
};

function NotificationModal({ notif, onClose, onSave }) {
    const [form, setForm] = useState(notif || {
        title: '', content: '', type: 'Hệ thống', targetType: 'All', priority: 'Trung bình'
    });
                    ...
                    <div className="grid grid-cols-3 gap-3">
                        <div><label className="block text-xs font-semibold text-gray-600 mb-1">Loại</label>
                            <select className={INPUT} value={form.type} onChange={e => set('type', e.target.value)}>
                                {['Hệ thống', 'Hoạt động', 'Nhắc nhở'].map(t => <option key={t} value={t}>{t}</option>)}
                            </select>
                        </div>
                        <div><label className="block text-xs font-semibold text-gray-600 mb-1">Đối tượng</label>
                            <select className={INPUT} value={form.targetType} onChange={e => set('targetType', e.target.value)}>
                                {['All', 'Branch', 'Cell', 'Individual'].map(t => <option key={t} value={t}>{t}</option>)}
                            </select>
                        </div>
                        <div><label className="block text-xs font-semibold text-gray-600 mb-1">Ưu tiên</label>
                        ...
                        : notifs.length === 0 ? <div className="text-center py-12 text-gray-400 text-sm">Chưa có thông báo nào</div>
                            : <table className="w-full text-sm">
                                <thead><tr className="bg-gray-50 border-b border-gray-200 text-xs font-semibold uppercase text-gray-500">
                                    <th className="px-4 py-3 text-left">Tiêu đề</th><th className="px-4 py-3 text-left">Loại</th><th className="px-4 py-3 text-left">Đối tượng</th><th className="px-4 py-3 text-left">Ưu tiên</th><th className="px-4 py-3 text-left">Trạng thái</th><th className="px-4 py-3 text-left">Thao tác</th>
                                </tr></thead>
                                <tbody>
                                    {notifs.map(n => {
                                        ...
                                        return (
                                            <tr key={n.id} className="border-b border-gray-100 hover:bg-gray-50 transition">
                                                <td className="px-4 py-3">
                                                    ...
                                                    <p className="text-xs text-gray-400 mt-0.5 truncate max-w-[240px]">{n.content}</p>
                                                </td>
                                                <td className="px-4 py-3 text-gray-500 text-xs">{n.type}</td>
                                                ...
                                                <td className="px-4 py-3"><span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${pr.cls}`}>{n.priority}</span></td>
                                                <td className="px-4 py-3"><span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${st.cls}`}>{n.status}</span></td>
